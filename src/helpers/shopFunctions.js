...
export const prices = async () => {
  ...
  const getPrice = getSavedCartIDs();
  const resultPrice = await getPrice.reduce(async (acc, newValue) => {
    const { price } = await fetchProduct(newValue);
    const result = await acc + price;
    return result;
  }, 0);
...
export const createCartProductElement = ({ id, title, price, pictures }) => {
  ...
  li.addEventListener('click', prices);
  return li;
};
